...
export function ScanResult({ result, onClose, onNewScan, onItemIssued }: ScanResultProps) {
  ...
  const handleMarkItemIssued = async (itemType: ItemType) => {
    ...
    try {
      const response: ItemIssuanceResponse = await markItemIssued(userData.userID, itemType)
      ...
      if (response.success) {
        ...
        success(`${itemType === ItemType.LUNCH_COUPON ? 'Lunch Coupon' : 'Bag'} marked as issued successfully!`, 'Item Updated')
        
        // Call the callback to update scan history
        if (onItemIssued) {
          onItemIssued(updatedUser)
        }
                ...
                <div className="grid grid-cols-1 gap-3">
                  <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        size="sm"
                        variant={userData.lunchCouponIssued ? "destructive" : "default"}
                  ...
                  <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        size="sm"
                        variant={userData.bagIssued ? "destructive" : "default"}
